...
public:
    ...
    bool WriteZBITSZSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZBITSZSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZBITSZSeed_deprecated(uint256& seed);
    bool EraseZBITSZSeed();
    bool EraseZBITSZSeed_deprecated();

    bool WriteZBITSZCount(const uint32_t& nCount);
    bool ReadZBITSZCount(uint32_t& nCount);
