...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZBITSZModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zbitszTracker->HasMintTx(hash))) {
            //zBITSZ stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZBITSZ;
...
bool IsZBITSZType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZBITSZ:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZBITSZ || type == TransactionRecord::MNReward) {
