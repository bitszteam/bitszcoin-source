...
#include "ui_zbitszcontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zbitszTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zbitszTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zBITSZ.";
            else if (!mint.isSeedCorrect)
                strReason = "The zBITSZ seed used to mint this zBITSZ is not the same as currently hold in the wallet";
            else
