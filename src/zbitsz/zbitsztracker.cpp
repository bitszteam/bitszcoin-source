...
CzBITSZTracker::CzBITSZTracker(std::string strWalletFile)
{
...
CzBITSZTracker::~CzBITSZTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzBITSZTracker::Init()
{
...
bool CzBITSZTracker::Archive(CMintMeta& meta)
{
...
bool CzBITSZTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzBITSZTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzBITSZTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzBITSZTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzBITSZTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzBITSZTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzBITSZTracker::GetSerialHashes()
{
...
CAmount CzBITSZTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzBITSZTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzBITSZTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzBITSZTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzBITSZTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzBITSZTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzBITSZTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzBITSZTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzBITSZTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzBITSZTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzBITSZTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzBITSZWallet* zBITSZWallet)
{
    bool iszBITSZWalletInitialized = (NULL != zBITSZWallet);
    CMintMeta meta;
    ...
    if (! iszBITSZWalletInitialized)
        zBITSZWallet = new CzBITSZWallet(strWalletFile);
    meta.isSeedCorrect = zBITSZWallet->CheckSeed(dMint);
    if (! iszBITSZWalletInitialized)
        delete zBITSZWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzBITSZTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBITSZTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzBITSZTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzBITSZTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzBITSZTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzBITSZTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzBITSZWallet* zBITSZWallet = new CzBITSZWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zBITSZWallet);
        }
        delete zBITSZWallet;
        LogPrint("zero", "%s: added %d dzbitsz from DB\n", __func__, listDeterministicDB.size());
    }
