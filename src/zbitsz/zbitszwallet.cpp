...
CzBITSZWallet::CzBITSZWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zbitsz seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBITSZSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBITSZSeed_deprecated()) {
                    LogPrintf("%s: Updated zBITSZ seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zbitsz seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zbitsz wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzBITSZWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBITSZCount(nCountLastUsed);
    else if (!walletdb.ReadZBITSZCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBITSZWallet::Lock()
{
    seedMaster = 0;
}

void CzBITSZWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBITSZWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBITSZ(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBITSZWallet::LoadMintPoolFromDB()
{
...
void CzBITSZWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBITSZWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBITSZWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zbitszTracker->HasPubcoinHash(pMint.first)) {
...
bool CzBITSZWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBITSZ(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zbitszTracker which also adds to database
    pwalletMain->zbitszTracker->Add(dMint, true);
...
void CzBITSZWallet::SeedToZBITSZ(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBITSZWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBITSZWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZBITSZCount(nCountLastUsed);
...
void CzBITSZWallet::GenerateDeterministicZBITSZ(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBITSZWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBITSZWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzBITSZWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
