...
// The zBITSZ block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zBITSZ is a hash of the serial
...
{
    //Create an output returning the zBITSZ that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZBITSZOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zBITSZ output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zBITSZ", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZBITSZOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zBITSZ output", __func__);
...
{
    CzBITSZTracker* zbitszTracker = pwallet->zbitszTracker.get();
    CMintMeta meta;
    if (!zbitszTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zbitszTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a BITSZ stake is the outpoint
