...
BOOST_AUTO_TEST_CASE(zerocoin_wrapped_serial_spend_test)
{
    ...
    CzBITSZWallet *czBITSZWallet = new CzBITSZWallet(wallet.strWalletFile);
    ...
    for (unsigned int i = 0; i < TESTS_COINS_TO_ACCUMULATE; i++) {
        ...
        czBITSZWallet->GenerateDeterministicZBITSZ(denom, coin, dMint, true);
        czBITSZWallet->UpdateCount();
